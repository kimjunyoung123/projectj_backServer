...
public interface storeDao extends JpaRepository<storeVo,Integer> {
    ...
    @Query(value = "select a.*,b.store_review_id,b.store_review_created,b.store_review_text,b.store_review_writer,(select count(*) from store_reviews where store_id=a.store_id)totalCount"
    +" from stores a left join store_reviews b on a.store_id=b.store_id"
    +" where a.store_address=? and a.store_name=? order by b.store_review_id desc limit ?,?",nativeQuery = true)
    List<Map<String,Object>> findJoinReviewsSaddressAndSname(String storeAddress,String storeName,int start,int pageSize);
