...
public interface userdao extends JpaRepository<userVo,Integer>{
    ...
    @Query(value = "select (select count(*) from users where uphone=?)up,(select count(*) from compnays where cphone=?)",nativeQuery = true)
    Map<String,Object>findByPhoneUsersAndCompanys(String phone,String sam_phone);

    @Query(value = "select (select count(*) from users where uemail=?)ue,(select count(*) from companys where cemail=?)",nativeQuery = true)
